...
- $$\hat{V}(b) = \underset{a\in\mathbb{A}}{\max}\sum_{s\in \mathbb{S}}b(s)\Big[r(s,a)+\gamma\sum_{s'\in\mathbb{S}}P(s'\vert s,a)\underset{\alpha_i\in\Tau_i}{\max}\alpha_i(s') \Big]$$，这里少乘了一个观测概率的矩阵以及对所有观测的求和，而这个求和小于1，所以，QMDP会产生一个高估的上界。
